...
version: 
Author: Yang Zhong
Date: 2024-06-24 11:33:03
LastEditors: Yang Zhong
LastEditTime: 2024-08-21 19:10:12
'''
...
class RadialBasisEdgeEncoding(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        elif type(basis).__name__.split(".")[-1] == 'ExponentialGaussianRadialBasisFunctions':
            num_basis = basis.num_basis_functions
        elif type(basis).__name__.split(".")[-1] == 'ExponentialBernsteinRadialBasisFunctions':
            num_basis = basis.num_basis_functions
        elif type(basis).__name__.split(".")[-1] == 'GaussianRadialBasisFunctions':
            num_basis = basis.num_basis_functions
        elif type(basis).__name__.split(".")[-1] == 'BernsteinRadialBasisFunctions':
            num_basis = basis.num_basis_functions
        else:
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:        
        ...
        data[AtomicDataDict.EDGE_VECTORS_KEY] = edge_dir
